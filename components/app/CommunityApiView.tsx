...
interface CommunityApiViewProps {
  ...
  runDeleteArticleAPI?: (articleId: number) => void;
}
                  ...
                  >
                    <div className="space-y-1 overflow-hidden pr-3">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 pt-1.5">
                        <span className="text-[9px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded font-bold">
                          Writer: {art.user_id || "system"}
                        </span>
            ...
            <div className="space-y-5">
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  {runDeleteArticleAPI && (
                    <button
                      onClick={() => runDeleteArticleAPI(activeArticle.id)}
                      className="px-2.5 py-1 text-[11px] font-bold text-red-600 hover:text-white border border-red-200 hover:bg-red-600 rounded-lg transition-colors cursor-pointer outline-none shrink-0"
                    >
                      Delete Topic
                    </button>
                  )}
                </div>
                <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-wrap">{activeArticle.article_content}</p>
                
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-[10px] text-[#9CA3AF] mt-4 pt-3 border-t border-gray-200">
                  <div className="flex flex-wrap items-center gap-2.5">
                    <span className="font-mono">ARTICLE ID: {activeArticle.id}</span>
                    <span>•</span>
                    <span className="font-mono bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-bold">WRITER: {activeArticle.user_id || "system"}</span>
                  </div>
